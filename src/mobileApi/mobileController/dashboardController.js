...
export const getVillageDetails = catchAsyncError(async (req, res, next) => {
  try {
    ...
    const villagesDetails = await beneficiarDetails.find().populate({
      path: "khatauniId",
      select:
        "khatauniSankhya khasraNumber acquiredKhasraNumber areaVariety acquiredRakbha isAllDocumentSubmitted",
    });
    ...
    for (const village of villagesDetails) {
      ...
      const villageData = villages.find(
        (v) => v._id.toString() === village.villageId.toString()
      );
      const villageName = villageData
      ...
      const beneficiariesForKhatauni = await beneficiarDetails.find({
        khatauniId: khatauni._id,
      });
      const concatBeneficiaries = beneficiariesForKhatauni
        .map((b) => b.beneficiaryName || "N/A")
        .join(", ");
      groupedVillages[khatauniSankhya].concatBeneficiaries =
        concatBeneficiaries;
      ...
      for (const beneficiary of beneficiariesForKhatauni) {
        ...
        const docDetail = docDetails.find(
          (doc) =>
            doc.khatauniSankhya === khatauniSankhya &&
            doc.beneficiaryName === beneficiaryName
        );
        ...
        if (
          !groupedVillages[khatauniSankhya].beneficiaries.some(
...
export const getBeneficiariesByKhatauniSankhya = catchAsyncError(
  async (req, res, next) => {
    try {
      ...
      const villages = await khatauniDetailsWeb
        .find({
      ...
      if (!villages || villages.length === 0) {
        return next(
          new ErrorHandler(
            "No beneficiaries found for the provided Khatauni Sankhya.",
            200
          )
        );
      }
      ...
      if (!beneficiaries || beneficiaries.length === 0) {
        return next(
          new ErrorHandler(
            "No beneficiaries found in beneficiary details.",
            200
          )
        );
      }
      ...
      const beneficiariesResponse = await Promise.all(
        beneficiaries.map(async (beneficiary) => {
          ...
          const docs = await beneficiaryDocs
            .find({ beneficiaryId: beneficiary._id })
            .exec();
          const documentsDetails = {
            accountNumber: docs[0]?.accountNumber
              ? `uploaded on ${formatDate(docs[0].createdAt)}`
              : "",
            ifscCode: docs[0]?.ifscCode
              ? `uploaded on ${formatDate(docs[0].createdAt)}`
              : "",
            aadhaarNumber: docs[0]?.aadhaarNumber
              ? `uploaded on ${formatDate(docs[0].createdAt)}`
              : "",
            panCardNumber: docs[0]?.panCardNumber
              ? `uploaded on ${formatDate(docs[0].createdAt)}`
              : "",
            remarks: docs[0]?.remarks
              ? `uploaded on ${formatDate(docs[0].createdAt)}`
              : "",
            photo: docs[0]?.photo
              ? `uploaded on ${formatDate(docs[0].createdAt)}`
              : "",
            landIndemnityBond: docs[0]?.landIndemnityBond
              ? `uploaded on ${formatDate(docs[0].createdAt)}`
              : "",
            structureIndemnityBond: docs[0]?.structureIndemnityBond
              ? `uploaded on ${formatDate(docs[0].createdAt)}`
              : "",
            uploadAffidavit: docs[0]?.uploadAffidavit
              ? `uploaded on ${formatDate(docs[0].createdAt)}`
              : "",
            aadhaarCard: docs[0]?.aadhaarCard
              ? `uploaded on ${formatDate(docs[0].createdAt)}`
              : "",
            panCard: docs[0]?.panCard
              ? `uploaded on ${formatDate(docs[0].createdAt)}`
              : "",
            chequeOrPassbook: docs[0]?.chequeOrPassbook
              ? `uploaded on ${formatDate(docs[0].createdAt)}`
              : "",
          };

          // Check if all document values are empty
          const shouldRemoveDocumentsKey = Object.values(
            documentsDetails
          ).every((value) => value === "");
          ...
          return {
            ...
            ...(shouldRemoveDocumentsKey
              ? {}
              : { documents: documentsDetails }), // Include documents only if not empty
          };
        })
      );
...
export const uploadDocs = async (req, res) => {
  try {
    ...
    if (!Array.isArray(beneficiaries)) {
      return res
        .status(400)
        .json({ status: false, message: "Beneficiaries should be an array." });
    }
      ...
      beneficiaries.map(async (beneficiary, index) => {
        ...
        const beneficiaryData = {
          ...
          accountNumber:
            ...
            (existingDoc ? existingDoc.accountNumber : ""),
          ifscCode:
            beneficiary.ifscCode || (existingDoc ? existingDoc.ifscCode : ""),
          aadhaarNumber:
            beneficiary.aadhaarNumber ||
            (existingDoc ? existingDoc.aadhaarNumber : ""),
          panCardNumber:
            beneficiary.panCardNumber ||
            (existingDoc ? existingDoc.panCardNumber : ""),
          remarks:
            beneficiary.remarks || (existingDoc ? existingDoc.remarks : ""),
          isConsent1: beneficiary.isConsent1 === "true",
          isConsent2: beneficiary.isConsent2 === "true",
          photo:
            extractFileName("photo", index) ||
            (existingDoc ? existingDoc.photo : ""),
          landIndemnityBond:
            extractFileName("landIndemnityBond", index) ||
            (existingDoc ? existingDoc.landIndemnityBond : ""),
          structureIndemnityBond:
            extractFileName("structureIndemnityBond", index) ||
            (existingDoc ? existingDoc.structureIndemnityBond : ""),
          uploadAffidavit:
            extractFileName("uploadAffidavit", index) ||
            (existingDoc ? existingDoc.uploadAffidavit : ""),
          aadhaarCard:
            extractFileName("aadhaarCard", index) ||
            (existingDoc ? existingDoc.aadhaarCard : ""),
          panCard:
            extractFileName("panCard", index) ||
            (existingDoc ? existingDoc.panCard : ""),
          chequeOrPassbook:
            extractFileName("chequeOrPassbook", index) ||
            (existingDoc ? existingDoc.chequeOrPassbook : ""),
          khatauniSankhya: khatauniSankhya,
        };
    ...
    const updatedBeneficiaries = await Promise.all(
      processedBeneficiaries.map(async (beneficiaryDoc) => {
        ...
        const allDocsUploaded = requiredFields.every(
          (field) =>
            beneficiaryDoc[field] && beneficiaryDoc[field].trim() !== ""
        );
        beneficiaryDoc.documentUploadedEach = allDocsUploaded
          ? "completed"
          : "incomplete";
        ...
        if (
          beneficiaryDoc.documentUploadedEach === "completed" &&
          beneficiaryDoc.isConsent1 &&
          beneficiaryDoc.isConsent2
        ) {
    ...
    const submissionStatus = allDocsFilledForAllBeneficiaries
      ? "Completed"
      : "Partial";
