...
const createVillage = async (req, res) => {
  const { villageName, villageNameHindi, villageCode, villageArea, khatauni, totalBeneficiaries, villageStatus } = req.body;
...
const bulkCreateVillages = async (req, res) => {
  ...
  if (!Array.isArray(villages) || villages.length === 0) {
    return res.status(400).json({ success: false, message: 'Please provide a valid array of villages.' });
  }
...
const updateVillageById = async (req, res) => {
  ...
  try {
    const updatedVillage = await Village.findByIdAndUpdate(id, {
      villageName,
      villageNameHindi,
      villageCode,
      villageArea,
...
export { createVillage, bulkCreateVillages, getAllVillages, getVillageById, updateVillageById, deleteVillageById };
